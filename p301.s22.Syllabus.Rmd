...
title: |
  ...
  Syllabus for PSYC 301:
subtitle: |
 Elementary Statistics for Psychology (Spring, 2022)
...
## Exams 
A <- 75 # Points per exam
B <- 4 # Number of Exams

## Homework
U <- 5 # Points per easier HW
V <- 4 # Number of easier HWs
W <- 8 #Points per harder HW
X <- 7 # Number of harder HWs
Y <- 12 # Points per hardest HW
Z <- 2 # Number per hardest HW

############## Lab ##################

## Attendance
C <-  5 # Points per attendance
D <- 13 # Number of lab meetings

## Support tasks
E <- 2 # Points per support task
G <- 8 # Number of support Tasks

## Rough Drafts
H <- 25 # Points per Rough Drafts (Peerceptiv)
I <-  4 # Number of Rough Drafts

## Final Dfts
J <- 44 # Points for Final Draft 1 (Turnitin)
K <- 55 # Points for Final Draft 2 (Turnitin)
L <- 45 # Points for Final Draft 3 (Turnitin)
M <- 75 # Points for Final Draft 4 (Turnitin)

## POINT TOTALS
N <- sum((A*B),(C*D),(E*G),(H*I),J,K,L,M,(U*V),(W*X),(Y*Z)) # For the entire class
O <- sum(      (C*D),(E*G),(H*I),J,K,L,M) # For the lab only
P <- sum(            (E*G),(H*I),J,K,L,M) # For the research project Only
Q <- sum(                  (H*I),J,K,L,M) # For all the drafts (rough & final) only
R <- sum(                        J,K,L,M) # For the final drafts only
...
## 
Date_Exam1_Start <- "Thursday, 16 September"
Time_Exam1_Start <- "2 pm"
Date_Exam1_Finish <- "Tuesday, 21 September"
Time_Exam1_Finish <- "9:35 am"

## Exam 2
Date_Exam2_Start <- "Thursday, 7 October"
Time_Exam2_Start <- "2 pm"
Date_Exam2_Finish <- "Tuesday, 12 October"
Time_Exam2_Finish <- "9:35 am"

## Exam 3
Date_Exam3_Start <- "Thursday, 11 November"
Time_Exam3_Start <- "2 pm"
Date_Exam3_Finish <- "Tuesday, 16 November"
Time_Exam3_Finish <- "9:35 am"

## Final Exam
Date_Final_Start <- "Monday, 10 December"
Time_Final_Start <- "8:00 am"
Date_Final_Finish <- "Wednesday, 15 December"
Time_Final_Finish <- "4:30 pm"

# Homework problems
Name_HW01 <- "Central Tendency"
Date_HW01_Start <- "Monday, 9 September"
Time_HW01_Start <- "12 am"

Name_HW02 <- "Variation"
Date_HW02_Start <- "Monday, 13 September"
Time_HW02_Start <- "12 am"

Name_HW03 <- "z-scores"
Date_HW03_Start <- "Monday, 13 September"
Time_HW03_Start <- "12 am"

Name_HW04 <- "Boxplots"
Date_HW04_Start <- "Monday, 20 September"
Time_HW04_Start <- "12 am"

Name_HW05 <- "Standard Error and Confidence Intervals"
Date_HW05_Start <- "Monday, 27 September"
Time_HW05_Start <- "12 am"

Name_HW06 <- "Area Under Curve"
Date_HW06_Start <- "Monday, 27 September"
Time_HW06_Start <- "12 am"

Name_HW07 <- "Chi-square"
Date_HW07_Start <- "Monday, 11 October"
Time_HW07_Start <- "12 am"

Name_HW08 <- "Independent-samples t-test"
Date_HW08_Start <- "Monday, 18 October"
Time_HW08_Start <- "12 am"

Name_HW09 <- "ANOVA table"
Date_HW09_Start <- "Monday, 25 October"
Time_HW09_Start <- "12 am"

Name_HW10 <- "ANOVA Sums of Squares"
Date_HW10_Start <- "Monday, 25 October"
Time_HW10_Start <- "12 am"

Name_HW11 <- "Correlation"
Date_HW11_Start <- "Monday, 1 November"
Time_HW11_Start <- "12 am"

Name_HW12 <- "Simple regression"
Date_HW12_Start <- "Monday, 8 November"
Time_HW12_Start <- "12 am"

Name_HW13 <- "Paired-samples t-test"
Date_HW13_Start <- "Monday, 15 November"
Time_HW13_Start <- "12 am"
...
# Support Tasks
Name_SupportTask1 <- "Topic Brainstorm"
Date_SupportTask1 <- "Saturday, 4 September"
Time_SupportTask1 <- "5 pm"

Name_SupportTask2 <- "Finalize Study Designs"
Date_SupportTask2 <- "Friday, 17 September at 11:59 pm"

Name_SupportTask3 <- "Full Dataset for Semester"
Date_SupportTask3 <- "Friday, 1 October at 11:59 pm"

Name_SupportTask4 <- "Statistical Output for Method section"
Date_SupportTask4 <- "Friday, 8 October at 11:59 pm"

Name_SupportTask5 <- "Statistical Output for t-test"
Date_SupportTask5 <- "Friday, 22 October at 11:59 pm"

Name_SupportTask6 <- "Statistical Output for ANOVA"
Date_SupportTask6 <- "Friday, 29 October at 11:59 pm"

Name_SupportTask7 <- "Statistical Output for Correlation"
Date_SupportTask7 <- "Friday, 12 November at 11:59 pm"

Name_SupportTask8 <- "Statistical Output for Regression"
Date_SupportTask8 <- "Friday, 12 November at 11:59 pm"

# Rough Drafts
Date_RoughDraft1 <- "by 10:00 am, the day of your lab, 20/22 September"
Date_RoughDraft2 <- "by 10:00 am, the day of your lab, 11/13 October"
Date_RoughDraft3 <- "by 10:00 am, the day of your lab, 1/3 November"
Date_RoughDraft4 <- "by 11:59 pm, 19 November"

#### CHANGE THESE DATES TO PUT TIME BETWEEN ROUGH DRAFT###
# Final Drafts
Date_FinalDraft1 <- "Before your own lab begins, 4 or 6 October"
Date_FinalDraft2 <- "Before your own lab begins, 25 or 27 October"
Date_FinalDraft3 <- "Before your own lab begins, 15 or 17 November"
Date_FinalDraft4 <- "8 December by 11:59 pm"
...
CourseInfo2 <- CourseInfo %>% select(3:7)

CourseInfo2 %>%
  kbl(caption = "Section Information for PSYC 301, both taught by Dr. Patrick Bolger") %>%
...
PeopleInfo <- read_tsv("tables/PeopleInfo.tsv", na="") 

```

```{r PeopleInfoHtml, eval=T}
...
PeopleInfo2 %>% 
  ...
  pack_rows("Lab TAs for Section 989, Mondays, 10am-12pm, Milner 202", 2, 3, background = "#D6D3C4") %>%
  pack_rows("Lab TA for Section 990, Mondays, 12-2pm, Milner 202", 4, 4, background = "#D6D3C4") %>%
  pack_rows("Lab TA for Section 991, Mondays, 2-4pm, Milner 202", 5, 5, background = "#D6D3C4") %>%
  pack_rows("Lab TAs for Section 992, Mondays, 4-6pm, Milner 202", 6, 7, background = "#D6D3C4") %>%
  pack_rows("Lab TA for Section 994, Wednesdays, 10-12pm, Milner 202", 8, 8, background = "#D6D3C4") %>%
  pack_rows("Lab TAs for Section 995, Wednesdays, 12-2pm, Milner 202", 9, 10, background = "#D6D3C4") %>%
  pack_rows("Lab TA for Section 996, Wednesdays, 2-4pm, Milner 202", 11, 11, background = "#D6D3C4") %>%
  pack_rows("Lab TA for Section 998, Wednesdays, 4-6pm, Milner 202", 12, 12, background = "#D6D3C4") %>%
  pack_rows("Undergraduate Teaching Scholar for Sections 989-992", 13, 13, background = "#D6D3C4") %>%
  pack_rows("Undergraduate Teaching Scholar for Sections 994-996; 998", 14, 14, background = "#D6D3C4") %>%
  row_spec(0:0, bold = T, color = "white", background = "#500000") %>%
  kable_styling(bootstrap_options = "responsive", font_size=14, full_width = F, position = "left")
...
$\dagger\dagger$ Exceptions are during the week of 6-10 September, when Dr. Bolger will hold office hours on Monday, 6 September, from 10am-12pm, and during the week of 4-8 October, when he will also hold office hours on Monday, 4 October, from 10am-12pm.
...
Accordingly, since it is a 4-unit "W" course, at least 25% of your grade must be based on writing. In this class, it is `r paste0(round((R/N)*100,0),"%")` ***(`r R` points out of `r N`)***. 

Additionally, as per the rules of the [Department of Psychological and Brain Sciences](https://liberalarts.tamu.edu/psychology/), you must pass the writing portion of the course with at least 60% of the points on the writing assignments. For this class, this means that **your total summed score across all final drafts of papers must reach at least `r round(R*.6,0)` out of `r R` points.** 

**Failure to obtain `r round(R*.6,0)` points for the four final drats will result in a failure to pass _PSYC 301_ altogether, irrespective of your cumulative score out of `r N` for the entire class (main class + lab).**
...
This lab is designed to complement the lecture portion of *PSYC 301*. In lecture, you will learn the logic behind various statistical tests, how they are computed, and how to interpret them. 

In lab, you will apply this knowledge and learn both to run statistical tests (using software) and to write about the results for professional reports. This is similar to what psychological researchers do every day (in part). 
...
Again, these will be carried out in groups of about 4-6 students, which are in turn determined by mutual scheduling constraints, as you express them in a survey during the first week of class.
...
Support tasks, writing assignments, and research-group lab tasks are worth just under `r paste0(round((O/N)*100,0),"%")` of your total grade(or `r paste0(O,"/",N)`). 
...
Meetings for the all the labs are weekly and face-to-face. If students need to quarantine, we have *Zoom* on all the computers in Milner 202 so that groups can still meet.
...
There will be four exams (the 4th being the ***cumulative*** final exam), each worth `r A` points (summing to `r A*B` points, or `r paste0(round(((A*B)/N)*100,0),"%")` of your total grade). 

Exams will include calculations and multiple-choice questions. Exams 1-3 (but not the final) will be explicitly focused on the topics covered in just that section of the course. ***But note again that the final exam is cumulative***.
...
The translation for you is that in this course, you can re-take each exam up to 3 times within the exam date-interval, starting on the calendar day ***after*** your 1st attempt (if you request it on the Google Form that will be provided). We will accept your highest score of the 1-to-3 attempts on each exam that you take/re-take.

**However**, there are some ground rules to this process. Details are provided below:  


1. The earliest you can re-take any exam is the next calendar day.  

2. To re-take the exam, you will need to indicate your intention on a *Google Forms* survey that we will provide. From there, we will set things up so that you can re-take the exam starting the calendar day after you last attempted it. Do this as early as possible after you take the exam since we will have 24 hours to respond to your survey response.  

3. We will take your **highest** score as your final score for that exam      
Remember, if you do poorly on any exam, you just take it again. These are a little more like formative exams, and less like summative exams. It works in your favor to study between attempts, however. We hope you feel that the benefit of multiple attempts is worth the extra trouble.
...
## Homework Problems in Main Class

The homework problems$^{\dagger\dagger}$ assigned in the main class involve calculations. They will appear online in during the times indicated in the table below and on the course schedule (section \@ref(MainClassSchedule)), and you have until the end of the semester to finish them (`r Date_HW_all_Due` at `r Time_HW_all_Due`). However, you probably do not want to wait that long. They can be quite involved, and will probably take you between 1/2 hour and an hour to complete (especially the more difficult ones).

You also have to get ***all*** the answers right. This can be a little frustrating. If you don't get them all right on any given attempt, you just start over (but you will get a different data set every time you re-try the problem). You have an unlimited number of attempts.

These attempts ***do*** get recorded using the *Respondus Lockdown Browser*. This is just to deter having someone else do the problem for you, or help you do it. But it is not an exam since there are unlimited attempts.

There are thirteen of them. See below.

$\dagger\dagger$ Note that these ***used*** to be the short-answer/calculation problems on previous exams. The exams themselves are now purely multiple choice.
...
kbl(caption = 'Homework Problems by Date') %>%
  pack_rows("Monday, 6 September at 12 am", 1, 1, background = "#D6D3C4") %>%
  pack_rows("Monday, 13 September at 12 am", 2, 3, background = "#D6D3C4") %>%
  pack_rows("Monday, 20 September", 4, 4, background = "#D6D3C4") %>%
  pack_rows("Monday, 27 September", 5, 6, background = "#D6D3C4") %>%
  pack_rows("Monday, 11 October", 7, 7, background = "#D6D3C4") %>%
  pack_rows("Monday, 18 October", 8, 8, background = "#D6D3C4") %>%
  pack_rows("Monday, 25 October", 9, 10, background = "#D6D3C4") %>%
  pack_rows("Monday, 1 November", 11, 11, background = "#D6D3C4") %>%
  pack_rows("Monday, 8 November", 12, 12, background = "#D6D3C4") %>%
  pack_rows("Monday, 15 November", 13, 13, background = "#D6D3C4") %>%
...
There are `r O` points possible in the lab, the majority of which is carried by the research project (`r P`points), and a minority by weekly tasks carried out in lab (`r C*D` points).
...
- There will be `r G` *Support Tasks* that you turn in as a group, each worth `r E` for a total of `r E*G` points. The support tasks revolve around research design, data collection, preparation, and analysis.  
- You will also write somewhat simplified version of an [APA primary-research manuscript](https://tamu.libguides.com/apa). This will come in the form of four writing assignments, each of which in turn comprises a rough and a final draft.  
  - The `r I` rough drafts are each worth `r H` points (`r H*I` total), and are peer-reviewed (by your classmates) through [Peerceptiv](https://www.peerceptiv.com/).  
...
There are a total of `r G` *Support Tasks* Each is worth `r E` points, or `r E*G` points total. This amounts to `r paste0(round(((E*G)/N)*100,0),"%")` of the total final grade. 

Support Task 1 is done by individual, and has an absolute deadline. Support Tasks 2-8 are done in groups, and groups lose a point for each day these tasks are late.
...
Students will brainstorm on possible topics through a survey provided by the main instructor. When this survey is completed, the instructor will assign `r E` points to those who completed the survey on time. This is due at the latest on `r Date_SupportTask1` at **5 pm**.

<br/>

##### Design (1 task)  

Students will design their study in *Google Forms* in a *Google Drive* folder (specific folders shared out of the head instructor's *Google Drive* account).$^{\dagger\dagger}$ When the design is approved by the TA, the group members get `r E` points each. This is due `r Date_SupportTask2`.

$\dagger\dagger$This, and all subsequent tasks, will be associated with documents that are housed in a *Google Drive* folder that has been assigned to you in the head instructor's *Google Drive*. This is the only location considered official for your documents. Do not create any other *Google Drives* or use any other cloud-storage tools for this class.

<br/>

##### Data (1 task)  

The groups will collect data. When that data collection is complete and the data have been saved into a *jamovi* file (.omv), uploaded into the same Google Drive folder, and approved by the TA, each member of the group will get `r E` points. This support task is due `r Date_SupportTask3`.

<br/>

##### Outputs (5 tasks)  

There are four writing assignments, but the last three require several data-analysis steps. Writing Assignment #2 requires one data analysis, and the last two writing assignments each require two data analyses. This sums to five outputs. Each of these statistical outputs will be added to the **same jamovi file** in the same Google Drive folder. Each group member will get `r E` points when each of these analyses is approved as adequate by the TA.

***IMPORTANT NOTE: Please do not put multiple .omv files in the Google Drive Folder. This makes it difficult for the TA to determine which is the correct one.***

Due dates:  

- Support task 4 is due `r Date_SupportTask4`  
- Support task 5 is due `r Date_SupportTask5`  
- Support task 6 is due `r Date_SupportTask6` 
- Support task 7 is due `r Date_SupportTask7`  
- Support task 8 is due `r Date_SupportTask8`  
...
There are `r I` rough drafts at `r H` points each, which is `r H*I` points total, or `r paste0(round(((H*I)/N)*100,0),"%")` of the class total.

Each rough  of the `r I` rough drafts serves as a draft for your peers to review online. As one of these peer review-**ers**, **you** will review four other papers. You will then use the feedback from others to revise your own final draft. 
...
***Note that the last of this is due on `r Date_RoughDraft4` at 11:59 pm for ALL sections. This is so that we can finish the final peer review without encroaching on Reading Day (24 November), or the Thanksgiving Holiday (25-26 November).***

Rough drafts must be turned in via [Peerceptiv](https://peerceptiv.zendesk.com/hc/en-us/articles/115004505373-What-is-Peerceptiv-) within *Canvas*. A special link is provided in *Canvas*. Note that you **must** access [Peerceptiv](https://www.peerceptiv.com/) assignments from within *Canvas* using that special link. **Do not** go to the [Peerceptiv](https://www.peerceptiv.com/) website to log in; this leads to problems. 

Turning in your rough draft late leads to accruing penalties in two ways:

1. Submitting your rough draft at any time ***between*** 10 am on the day of your lab and 10 am of the subsequent day is a submission during the "grace period". It will result in an automatic 50% off your peer-review grade (or `r round(H*.5,1)` our of `r H` points). If you upload your draft on time (or at least by the end of the grace period), the grading of the rough draft is done automatically through [Peerceptiv](https://www.peerceptiv.com/) as other students receive your draft and evaluate it.  

2. If you miss the grace period, then you will need to submit your draft to a *Turnitin* assignment that will appear after the grace period (It will be named something like *Late Rough Draft of Writing Assignment #X*). Your TA will then evaluate the paper using the same rubric, and the main instructor will apply the appropriate late penalties. More information is below.  

- Penalties   
  - First, your TA will be forced to evaluate your paper personally since you will be locked out from the peer-review process in *Peerceptiv*.$^{\dagger\dagger}$ The main instructor will apply the 50% penalty to the rough draft that [Peerceptiv](https://www.peerceptiv.com/) ***would*** have applied anyway had you uploaded it during the grace period.  
  - Second, you will receive **additional** cumulative 10% deductions off your **FINAL draft** for every 24-hour period (10 am one day to 10 am the next day) that the **ROUGH draft** is late after the grace period. This penalty accrues on your final draft (and not your rough draft) since (understandably) *Peerceptiv* will not allow you to upload a document **after the grace period**. The 10% penalty on your final draft accrues each day for 10 days after the grace period, at which point you can no longer get any points for the final draft (i.e., the penalties will have accrued to 100%). 
...
(varying points each [see bullet points below]; `r R` points total)  

- Final Draft of Writing Assignment #1:  `r J`/`r R` points (or `r paste0(round((J/R)*100,0),"%")`)  
- Final Draft of Writing Assignment #2:  `r K`/`r R` points (or `r paste0(round((K/R)*100,0),"%")`)  
- Final Draft of Writing Assignment #3:  `r L`/`r R` points (or `r paste0(round((L/R)*100,0),"%")`)  
- Final Draft of Writing Assignment #4:  `r M`/`r R` points (or `r paste0(round((M/R)*100,0),"%")`)  

<br/>

You will apply the feedback from your rough draft to your final (improved) draft. This is due **by the time your lab meets, usually two weeks after the peer review.** 

Specifically, they are due at 10 am on the following dates:  
...
Final drafts must be turned in via [Turnitin](https://help.turnitin.com/feedback-studio/blackboard/basic/student/student-category.htm), which will be linked on *Canvas*. Note that there is no need to access anything from the [Turnitin](https://www.turnitin.com/) website itself, except for help files, if you need them. Uploading your files to [Turnitin](https://www.turnitin.com/) directly (outside of *Canvas*) will cause problems.  

Turning in your final draft late accrues penalties daily. Specifically, 10% will be deducted from the grade of your final version for each 24-hour period the assignment is late after the initial deadline. These penalties will accrue for up to 10 days after the initial deadline, after which is will certainly be worth zero points (a 100% would have accrued). Note however, that if you have penalties carrying over from a rough draft turned in after the grace period (see above), fewer days will elapse before you reach a zero-point maximum on your final draft.
...
**NOTE THAT IRRESPECTIVE OF YOUR GRADE FOR THE OVERALL CLASS, YOUR SCORE SUMMED ACROSS THE FINAL DRAFTS MUST REACH AT LEAST `r R*.595` OUT OF `r R` POINTS IN ORDER TO PASS _PSYC 301_ (60% of `r R`).** 
...
Each final draft must reflect substantial changes from the respective rough draft, and these changes must both be commensurate with and accommodate the feedback on that rough draft. If we find that your have turned in a final draft that has not taken the feedback seriously, we reserve the right to return it to you to revise again. In this case, the late-submission clock re-sets itself to the day we notice the failure to make substantial changes. It continues to "tick" daily after that point, however.
...
##### Confirming your uploads 

[Peerceptiv](https://www.peerceptiv.com/) and [Turnitin](https://www.turnitin.com/) have different ways of confirming your uploads:  

1. For [Peerceptiv](https://www.peerceptiv.com/), make sure that it says “Document Submitted” (after submitting) [take a screenshot if you want proof]; if it doesn’t, **SUBMIT AGAIN**.  
2. For [Turnitin](https://www.turnitin.com/), please make sure that you get an email confirmation from [Turnitin](https://www.turnitin.com/); if you don't, **SUBMIT AGAIN**.  
...
(`r C*D` points [`r C` points $\times$ `r D` physical attendances], or `r paste0(round(((C*D)/N)*100,0),"%")` of the total grade)  
...
|:----------:|:-------------:|:--------------------:|:--------------------:|:------------:|:----:|
| Main Class | Exams | `r A` | `r B` | `r A*B` | `r paste0(round(((A*B)/N)*100,0),"%")` |
| Main Class | Easier HW Problems | `r U` | `r V` | `r U*V` | `r paste0(round(((U*V)/N)*100,0),"%")` |
| Main Class | Harder HW Problems | `r W` | `r X` | `r W*X` | `r paste0(round(((W*X)/N)*100,0),"%")` |
| Main Class | Hardest HW Problems | `r Y` | `r Z` | `r Y*Z` | `r paste0(round(((Y*Z)/N)*100,0),"%")` |
| Lab | Rough Drafts | `r H` | `r I` | `r H*I` | `r paste0(round(((H*I)/N)*100,0),"%")` |
| Lab | Final Drafts Assignment #1 | `r J` | 1 | `r J` | `r paste0(round((J/N)*100,0),"%")` |
| Lab | Final Drafts Assignment #2 | `r K` | 1 | `r K` | `r paste0(round((K/N)*100,0),"%")` |
| Lab | Final Drafts Assignment #3 | `r L` | 1 | `r L` | `r paste0(round((L/N)*100,0),"%")` |
| Lab | Final Drafts Assignment #4 | `r M` | 1 | `r M` | `r paste0(round((M/N)*100,0),"%")` |
| Lab | Support Tasks | `r E` | `r G` | `r E*G` | `r paste0(round(((E*G)/N)*100,0),"%")` |
| Lab | Attendance | `r C` | `r D` | `r C*D` | `r paste0(round(((C*D)/N)*100,0),"%")` |
|  |  |  | TOTALS: | `r N` | 100% |  
...
| Points on Final Drafts ($k$)  |   `r R*.595` $\le k$  |   $k \lt$ `r R*.595`  |
...
MainCalendar <- read_tsv("tables/ClassCal.tsv", na="") 

```



```{r diplayMainCalendarHtml, eval=T}

MainCalendar %>%
kbl(caption = 'Main Class Schedule by Week') %>%
...
pack_rows("Unit 3: Inferential statistics you need for your papers", 7, 11, background = "#D6D3C4") %>%
pack_rows("Unit 4: Other inferential statistics", 12, 15, background = "#D6D3C4") %>%
pack_rows("Final Exam", 16, 16, background = "#D6D3C4") %>%
row_spec(0:0, bold = T, color = "white", background = "#500000") %>%
kable_styling(bootstrap_options = "responsive", font_size=11, full_width = F, position = "left")
```
...
```{r diplayLabCalendarHtml, eval=T}
LabCalendar %>%
kbl(caption = 'Lab Schedule by Week') %>%
