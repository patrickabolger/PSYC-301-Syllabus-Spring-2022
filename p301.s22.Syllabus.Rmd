...
## 
Date_Exam1_Start <- "Friday, 11 February"
Time_Exam1_Start <- "5:00 pm"
Date_Exam1_Finish <- "Wednesday, 16 February"
Time_Exam1_Finish <- "11:59 pm"

## Exam 2
Date_Exam2_Start <- "Friday, 4 March"
Time_Exam2_Start <- "5:00 pm"
Date_Exam2_Finish <- "Wednesday, 9 March"
Time_Exam2_Finish <- "11:59 pm"

## Exam 3
Date_Exam3_Start <- "Friday, 8 April"
Time_Exam3_Start <- "5:00 pm"
Date_Exam3_Finish <- "Wednesday, 13 April"
...
#NEW
Date_FinalDraft1 <- "by 11:59 pm, Friday, 25 February"
Date_FinalDraft2 <- "by 11:59 pm, Friday, 25 March"
Date_FinalDraft3 <- "by 11:59 pm, Friday 15 April"
Date_FinalDraft4 <- "Tuesday, 3 May by 11:59 pm"
...
CourseInfo %>%
  select(3:7) %>%
  kbl(caption = "Main-Class Information for PSYC 301") %>%
...
LabInfo <- read_tsv("tables/LabInfo.tsv", na="NA") 

```

```{r LabInfoHtml, eval=T}

LabInfo %>%
  kbl(caption = "Section Information for PSYC 301") %>%
  pack_rows("Sections 902-905, Tuesdays in Milner 202", 1, 4, background = "#D6D3C4") %>%
  pack_rows("Sections 912-915, Wednesdays in Milner 202", 5, 8, background = "#D6D3C4") %>%
  row_spec(0:0, bold = T, color = "white", background = "#500000") %>%
  kable_styling(bootstrap_options = "responsive", font_size=14, full_width = F, position = "left")
...
```{r LabInfoNotHtml, results = 'asis', eval=F}

 pandoc.table(LabInfo, style = "multiline", split.table = Inf, missing = "")
...
Each of the `r RR` rough drafts serves as a draft for your peers to review online. As one of these peer review-**ers**, you will review four other papers. Subsequently, as a review-**ee**, you will use the feedback from others to revise your own final draft. 
...
**NOTE THAT IRRESPECTIVE OF YOUR GRADE FOR THE OVERALL CLASS, YOUR SCORE SUMMED ACROSS THE FINAL DRAFTS MUST REACH AT LEAST `r round(EEE*.595,0)` OUT OF `r EEE` POINTS IN ORDER TO PASS _PSYC 301_ (60% of `r EEE`).** 
...
|:----------:|:-------------:|:--------------------:|:--------------------:|:------------:|:----:|
| Main Class | Exams | `r AA` | `r BB` | `r AA*BB` | `r paste0(round(((AA*BB)/AAA)*100,1),"%")` |
| Main Class | Perusall Readings | `r II` | `r JJ` | `r II*JJ` | `r paste0(round(((II*JJ)/AAA)*100,1),"%")` |
| Main Class | Easier HW Problems | `r CC` | `r DD` | `r CC*DD` | `r paste0(round(((CC*DD)/AAA)*100,1),"%")` |
| Main Class | Harder HW Problems | `r EE` | `r FF` | `r EE*FF` | `r paste0(round(((EE*FF)/AAA)*100,1),"%")` |
| Main Class | Hardest HW Problems | `r GG` | `r HH` | `r GG*HH` | `r paste0(round(((GG*HH)/AAA)*100,1),"%")` |
| Lab | Rough Drafts | `r QQ` | `r RR` | `r QQ*RR` | `r paste0(round(((QQ*RR)/AAA)*100,1),"%")` |
| Lab | Final Draft Assignment 1 | `r SS` | 1 | `r SS` | `r paste0(round((SS/AAA)*100,1),"%")` |
| Lab | Final Draft Assignment 2 | `r TT` | 1 | `r TT` | `r paste0(round((TT/AAA)*100,1),"%")` |
| Lab | Final Draft Assignment 3 | `r UU` | 1 | `r UU` | `r paste0(round((UU/AAA)*100,1),"%")` |
| Lab | Final Draft Assignment 4 | `r VV` | 1 | `r VV` | `r paste0(round((VV/AAA)*100,1),"%")` |
| Lab | Support Tasks | `r OO` | `r PP` | `r OO*PP` | `r paste0(round(((OO*PP)/AAA)*100,1),"%")` |
| Lab | Perusall Videos | `r KK` | `r LL` | `r KK*LL` | `r paste0(round(((KK*LL)/AAA)*100,1),"%")` |
| Lab | Attendance | `r MM` | `r NN` | `r MM*NN` | `r paste0(round(((MM*NN)/AAA)*100,1),"%")` |
|  |  |  | TOTALS: | `r AAA` | 100% |  
...
kbl("html", escape = F, caption = 'Lab Schedule by Week') %>%
  ...
  kable_styling(bootstrap_options = "responsive", font_size = 11, full_width = F, position = "left") %>%
  row_spec(0:0, bold = T, color = "white", background = "#500000") %>%
  footnote(general = c("NF19 = Navarro & Foxcroft (2019) - your main textbook  ", "Bolger20 = Bolger (2020) - your lab manual  ", "All times refer to the day of your particular lab, unless otherwise specified  ", "However, note that all uploads to *Peerceptiv* are due at 9:00 AM on the day of your lab, no matter what time your lab actually meets  "))
```  
