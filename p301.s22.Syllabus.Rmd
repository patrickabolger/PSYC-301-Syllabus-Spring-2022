...
Support tasks, writing assignments, and research-group lab tasks are worth just under `r paste0(round((BBB/AAA)*100,0),"%")` of your total grade(or `r paste0(BBB,"/",AAA)`). 
...
The translation for you is that in this course, you can re-take each exam up to 3 times within the exam date-interval. **Note however that there is a 12-hour waiting period between each attempt, so if you finish your first attempt 24 hours before the deadline, you will only get 2 attempts, and if you finish your first attempt 12 hours before the deadline, you will only get one attempt.** We will accept your highest score of the 1-to-3 attempts on each exam that you take/re-take.
...
The homework problems$^{\dagger\dagger}$ assigned in the main class involve calculations. In the table below, you can see when will appear online, and when they are due. These are all Fridays, and the time that they are due is always 11:59 pm. These are also listed on the course schedule (section \@ref(MainClassSchedule)). Note that they can be quite involved, and will probably take you between 1/2 hour and an hour to complete (especially the more difficult ones).

You also have to get ***all*** the answers right. This can be a little frustrating. If you don't get them all right on any given attempt, you just start over (but you will get a different data set every time you re-try the problem). You have an unlimited number of attempts in that time period, except for the fact that you can only attempt them once per day.
...
For each support task, there will be some extra questions you will need to answer. For support tasks 1-4, your answers will be on a separate form. For support tasks 5-8, you can answer the questions from within the .omv file in *jamovi*. 
...
Students will design their study in *Google Forms* in a *Google Drive* folder (specific folders shared out of the head instructor's *Google Drive* account).$^{\dagger\dagger}$ When the study is approved by the TA (due `r Date_ST2`), each group member will get a survey to fill out, which is due at the same time `r Date_ST2`. Based on the answers to the survey, the group members get up to `r OO` points each. 
...
The groups will collect data. When that data collection is complete and the data have been saved into a *jamovi* file (.omv), uploaded into the same *Google Drive* folder, and approved by the TA, each member of the group will also answer a survey. They will get up to `r OO` points for their answers. This support task is due `r Date_ST3`.

<br/>

##### Outputs (5 tasks)  

There are four writing assignments, but the last three require several data-analysis steps. Writing Assignment #2 requires one data analysis, and the last two writing assignments each require two data analyses. This sums to five outputs, and five support tasks, accordingly. 

For each of these last five support tasks, several questions will be given to students, and the students can answer each question within their own *jamovi* file itself. Students can get up to `r OO` points based on their answers, and the appropriateness of the respective analysis.
